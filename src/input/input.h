...
public:
    InputManager();
    ~InputManager();    // Set the camera to control
    void setCamera(Camera* camera);

    // Main update function - call every frame
    void update(float deltaTime);    // Process keyboard input
    ...
    void handleWindowMessage(UINT message, WPARAM wParam, LPARAM lParam);    // Set mouse capture
    void setMouseCapture(HWND hwnd, bool capture);    // Check if application should quit
    bool shouldQuit() const { return quit; }

    // Check if mouse is currently captured
    bool getMouseCaptured() const { return mouseCaptured; }    // Key state checking methods
